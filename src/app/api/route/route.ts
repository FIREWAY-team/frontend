...
> = {
  ...
  "pump-15": {
    // OSRM alt[1] = 자연스러운 대체 경로 · CCTV FAIL 골목만 회피한 느낌.
    // ⚠️ alt[2] 는 지도 밖으로 나가는 네모 우회. alt[0] 는 소형과 완전 겹침 → 대형 특성 X.
    //    alt[1] 이 "골목 진입 불가 → 인접 도로로 살짝 우회" 라는 CCTV 판정 반영에 가장 가깝다.
    osrmAltIndex: 1,
    passableProb: 0.42,
    passable: false,
    unresolved: true,
    label: "대형펌프차 · CCTV FAIL 골목 회피 · 대로변 접근",
  },
  "aerial-25": {
    osrmAltIndex: 1,
    passableProb: 0.72,
